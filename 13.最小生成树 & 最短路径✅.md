...
1. 输入一个加权连通图，其中顶点集合为V，边集合为E；
2. 初始化Vnew=x，其中x为集合V中的任一顶点（起始点），Enew=为空；
3. 重复以下操作直到Vnew=V：
    1. 在集合E中选取权值最小的边`<u, v>`，其中u为集合Vnew中的元素，而v不在Vnew中，并且v∈V（如果存在有多条满足上述条件且具有相同权值的边，则可以任选其中之一）；
    2. 将v加入Vnew中，同时将边`<u, v>`加入Enew中；
4. 输出Vnew和Enew构成的最小生成树；
