...
- Python 与 C 不同，在表达式内部赋值必须显式使用 [海象运算符](https://docs.python.org/zh-cn/3.11/faq/design.html#why-can-t-i-use-an-assignment-in-an-expression) `:=`，海象运算符可以赋值并同时返回变量的值， 这避免了 C 程序中常见的问题：要在表达式中写 `==` 时，却写成了 `=`；
