...
- 既然要寻找左边界，搜索范围就需要从右边开始，不断往左边收缩，也就是说即使我们找到了`nums[mid] == target`, 这个 mid 的位置也不一定就是最左侧的那个边界，我们还是要向左侧查找，也就是说，除了在` nums[mid] `偏大的时候需要收锁右边界，在 `nums[mid] `等于目标值的时候，不同于标准的二分查找直接返回，我们此时仍然需要继续收缩右边界；
- 划分` [left, mid]` 与` [mid + 1, right] `，**mid 被分到左边，**对应 `mid = left + (right - left) // 2`;  ->  **因为 mid` 对于长度为偶数的区间总是偏左的**
