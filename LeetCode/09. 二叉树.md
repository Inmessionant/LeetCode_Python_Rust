...
**多叉树  ->  有一种方法可以求任意节点的最近公共祖先：（假设求id4和id6最近公共祖先）**

- **对id4和id6的父节点（由近到远）列表进行记录，如parent_id4 = [4，3，0]，parent_id = [6，1，0]；**
- **寻找父节点列表交集，第一个出现的即为最近公共祖先，如[4，3，0] & [6，1，0] = [0]，则id0为id4和id6的最近公共祖先；**
