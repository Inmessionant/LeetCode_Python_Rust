...
permutations('ABCD', 2)
# AB AC AD BA BC BD CA CB CD DA DB DC  排列

combinations('ABCD', 2)
# AB AC AD BC BD CD 组合
...
- **一个行程中，如果航班处理不好容易变成一个圈，成为死循环；** -> 选择的时候`pop`出来，用完之后再`append `进去，`cur_to = tickets_dict[start].pop(0)，tickets_dict[start].append(cur_to)`
