...
```python
'''
    1. dfs(i, j， k)表示以i, j点为起点，最多可以移动k次球，将球移除边界的路径数；
    2. 递归条件：以当周围四个点为起点，最多可以移动k-1次球，将球移出边界，求和；
    3. 当(i, j)出界时，则路径数为1，当k = 0时且还没有出界时，则dfs(i, j, 0)无法出界；
'''
