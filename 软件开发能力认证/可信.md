...
class DirPermSystem:
    ...
    def __init__(self, path: List[int], statuses: List[int]):
        ...
        self.tree = [[index, parent] for index, parent in enumerate(path)]
    ...
    def grant_right(self, user_id: int, dir_id: int) -> None:
        ...
        if user_id not in self.user:
            ...
            self.user[user_id] = [dir_id]

        elif dir_id not in self.user[user_id]:

            self.user[user_id].append(dir_id)

 

    def remove_right(self, user_id: int, dir_id: int) -> bool:

        if user_id not in self.user or dir_id not in self.user[user_id]:

            return False

        self.user[user_id].remove(dir_id)
    ...
    def query_right(self, user_id: int, dir_id: int) -> bool:
        ...
        normal = [i[0] for i in self.tree if i[2] == 1]
    ...
    def query_num(self, user_id: int) -> int:
        ...
        return len(set(normal + right))

 

    def dfs(self, parent: int, callist):

        child = [i[0] for i in self.tree if i[1] == parent]
...
变量 tree 表示文件目录的树状结构，statuses 表示每个目录的访问权限，userRightMap 表示每个用户有权限的目录。

__init__ 完成对于文件目录树的初始化。根据输入的 path 数组构建出一个包含每个目录的 id、parent id 和状态的列表 tree，并根据 statuses 数组将每个目录的状态添加到 tree 中，最后初始化用户权限映射表 user。

change_status 根据输入的 dir_id 找到对应的目录，然后将其状态修改为 status。

grant_right 首先判断用户是否存在，如果不存在，则将其加入到用户权限映射表中，并将其赋予权限；如果存在，则将目录添加到其权限列表中。

remove_right 首先判断用户和目录是否存在于权限映射表中，如果不存在，则返回 False，否则从权限列表中删除指定的目录，并判断删除后该用户是否还有其他权限，如果没有则将其从用户权限映射表中删除。

query_right 首先判断指定目录是否属于“公开状态”（即访问状态为 1 的目录），如果是，则返回 True；否则，如果用户存在于权限映射表中，则通过 dfs 遍历该用户的权限列表中所有目录的子目录，将所有子目录添加到 right 列表中，并判断指定目录是否在 right 中。如果不在，则返回 False。

注意，并不是找到一个授权过的目录就停止查找，而是把所有授权过的目录及其子目录都添加到right中，最后才判断是否包含了dir_id。

query_num 首先遍历所有访问权限为 1 的目录，将其添加到 normal 列，然后通过dfs遍历该用户的权限列表中所有目录的子目录，将所有子目录添加到 right 列表中，最后返回 normal 和 right 的合集长度就是所有可访问的目录数量。
...
class Solution:
    ...
    def get_check_points(self, nodes: List[Tuple[int, List[int]]], key_nodes: List[int], org: int) -> List[int]:
        ...
        # nodes中每个元素为某节点及下游相邻节点信息：nodes[i][0]为节点id，nodes[i][1]为下游相邻节点nextNodes
        ...
        down_nodes = [org] if org in key_nodes else []

        while down_stack:

            cur_node = down_stack.pop()

            for down_node in node_dic[cur_node]:

                if down_node not in down_nodes:

                    down_stack.append(down_node)

                    down_nodes.append(down_node)

        up_stack = [org] if org in key_nodes else []

        up_nodes = [org]

        for node in down_nodes:

            if node in key_nodes and node not in up_nodes:

                up_stack.append(node)

                up_nodes.append(node)     

        while up_stack:

            cur_node = up_stack.pop()

            for node in nodes:

                if cur_node in node[1] and node[0] not in up_nodes:

                    up_stack.append(node[0])

                    up_nodes.append(node[0])
