...
class Solution:
    ...
    def init_num_to_loc(self, board):
        ...
        for i in range(bn):
            ...
            for j in range(bn):
                ...
                if board[i][j] not in num_to_loc:

                    num_to_loc[board[i][j]] = []

                num_to_loc[board[i][j]].append((i, j))

        return num_to_loc

 

    def init_row_col(self, bn):
    ...
    def earliest_completed_test(self, nums: List[int], board: List[List[int]]) -> int:
        ...
        row_n, col_n = self.init_row_col(bn)

        num_to_loc = self.init_num_to_loc(board)
        ...
        for num in nums:
            ...
            for i, j in loc_list:
                ...
                if row_n[i] == 0 or col_n[j] == 0:

                    return t

                if row_n[i] == 1 or col_n[j] == 1:

                    exist = True

                if exist and total >= bn:
