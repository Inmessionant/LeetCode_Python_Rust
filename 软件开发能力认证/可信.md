...
class FlowchartSys:
    ...
    def add_connection(self, connect_id: int, start_node_id: int, end_node_id: int) -> bool:
        ...
        if start_node_id == end_node_id or connect_id in self.connections:
    ...
    def remove_node(self, node_id: int) -> bool:
        ...
        self.connections = {cid: c for cid, c in self.connections.items() if node_id not in c}
    ...
    def query(self, node_id: int) -> List[int]:
        ...
        if node_id not in self.nodes:
            ...
            simple_node_ids = [c[1] for c in self.connections.values() if c[0] == node_id]

            hub_node_ids = [_id for _id, node_type in self.nodes.items() if node_type == 1 and _id != node_id]

            return sorted(list(set(simple_node_ids + hub_node_ids)))
```

该解法用了两个字典对象 nodes 和 connections 分别记录节点和连接。

l nodes的key=node_id, value=node_type

l connections的key=connect_id, value=元组(start_node_id, end_node_id)

在函数 remove_node 中，在移除节点之前，需要将包含节点node_id的所有连接也先删除。

查询函数遍历所有连接，将以 nodeId 为起点的所有连接的终点节点添加到simple_node_ids这个List中；

所有不等于node_id的中继节点添加到hub_node_ids。

然后合并两个List：simple_node_ids和hub_node_ids，转换为set后，再转换为List排序后返回。
