...
class StorageSystem:
    def __init__(self, cold_storage_num: int, cold_storage_price: int, normal_storage_num: int, \
        ...
        self.storage = {0: [cold_storage_num, cold_storage_price],
    ...
    def store(self, date: int, storage_id: int, storage_type: int, storage_days: int) -> int:
        self.count_date(date)
        if self.storage[storage_type][0]:
            self.order_success[storage_id] = [date, storage_type, storage_days]
            self.storage[storage_type][0] -= 1
            price = self.storage[storage_type][1] * storage_days
            return price
        elif storage_type == 1 and self.storage[1][0] == 0 and self.storage[0][0]:
            self.order_success[storage_id] = [date, 0, storage_days]
            self.storage[0][0] -= 1
            price = self.storage[0][1] * storage_days
            return price
        return -1

    def retrieve(self, date: int, storage_id: int) -> int:
        self.count_date(date)
        if storage_id in self.order_success:
            date1, storage_type, storage_days = self.order_success[storage_id]
            if date1 <= date <= date1 + storage_days:
                del self.order_success[storage_id]
                self.storage[storage_type][0] += 1
                self.order_retrive[storage_id] = [date1, storage_type, storage_days]
                return 0
            elif date1 + storage_days < date <= date1 + storage_days + self.delay:
                del self.order_success[storage_id]
                self.storage[storage_type][0] += 1
                self.order_retrive[storage_id] = [date1, storage_type, storage_days]
                price = self.storage[storage_type][1] * (date - date1 - storage_days)
                return price
        return -1

    def query(self, date: int):
        self.count_date(date)
        return [len(list(self.order_retrive.keys())), len(list(self.order_success.keys())),
                len(list(self.order_clear.keys()))]

    def count_date(self, date):
        for storage_id, order in self.order_success.items():
            # 判断是否超期, 需要清理
            if date > order[0] + order[2] + self.delay:
                del self.order_success[storage_id]
                # 冷库容量更新
                self.storage[order[1]][0] += 1
                self.order_clear[storage_id] = order
